class SchemaEditor {
    constructor(configuration, menuData, username, storage) {
        ...
        this.autoSaveFrequency = this.configuration.autoSave.frequency || 15000; // default 15 secondi
    ...
    executeMenuAction(action, size, bg) {
        const actions = {
            ...
            'saveWorkout': () => this.workoutManager.show(),
            ...
            'exportFormations': () => this.exportFormationSheet(),
            ...
            'rotateLeft': () => this.rotateSelected(-15),
            'rotateRight': () => this.rotateSelected(15),
            'rotateGroupLeft': () => this.rotateGroup(-15),
            'rotateGroupRight': () => this.rotateGroup(15),
            ...
            'togglePlayerNames': () => this.togglePlayerNames(),
            'snapToGrid': () => this.snapObjectsToGrid(),
            'renumberObjects': () => this.renumberAllObjects(),
            ...
            'setBackground': () => {
                if (bg) {
                    this.getCurrentTab().background = bg;
    ...
    syncMenuCheckboxes() {
        ...
        if (dashedCheck) dashedCheck.checked = this.dashedMode;
        ...
        if (borderCheck) borderCheck.checked = this.showCanvasBorder;
    }
    ...
    showSettingsDialog() {
        const dialog = createWindow({
            ...
            <div style="padding: 20px;">
                ...
                <div style="margin: 15px 0;">
                    ...
                    <input type="text" id="settings-author" value="${this.currentUser || ''}" 
                           style="width: 100%; padding: 8px; border: 1px solid #bdc3c7; border-radius: 4px;">
                </div>
                <div style="margin: 15px 0;">
        ...
        `,
            buttons: [
                {
                    label: 'Salva', onClick: () => {
                        this.currentUser = document.getElementById('settings-author').value;
                        alert('✅ Impostazioni salvate');
                    }
                }
            ]
    ...
    showTeamDialog() {
        this.teamManager.show();
    }

    hideTeamDialog() {
        this.teamManager.hide();
    }
    ...
    renumberAllObjects() {
        ...
        alert(`✅ ${sortedObjects.length} oggetti rinumerati`);
    }
    ...
    addObjectLabel(element, object) {
        ...
        // Mostra se la modalità è attiva
        const canvas = document.getElementById('canvas');
    ...
    toggleObjectLabels() {
        ...
        // Forza aggiornamento visibilità
...
["Step 1: ...", "Step 2: ...", ...]`;
        ...
        try {
            ...
            const response = await fetch('https://api.anthropic.com/v1/messages', {
                ...
                headers: {
                    ...
                    'x-api-key': 'YOUR_API_KEY_HERE', // ⚠️ INSERISCI LA TUA API KEY
            ...
            alert('✅ Step generati con successo!');

        } catch (error) {
            console.error('Errore generazione AI:', error);
            alert('❌ Errore nella generazione degli step: ' + error.message);
        } finally {
            const button = document.getElementById('generateAISteps');
            button.disabled = false;
            button.textContent = '✨ Genera Step con AI';
        }
    }
    ...
    addArrowEndpointControl(svg, arrow, type, position) {
        ...
        // Nascondi di default, mostra solo quando la freccia è selezionata
    ...
    showFrameProperties(frame) {
        ...
        // Prendi il primo oggetto per mostrare le proprietà (se ce ne sono più di uno, mostreremo quelle del primo)
    ...
    updateFrame() {
        ...
        // Aggiorna le proprietà dal form
        const updates = {
    ...
    updateCurrentFrame() {
        ...
        // Aggiorna solo gli oggetti che sono già nel frame
        frame.objects.forEach((objData, objId) => {
            const obj = tab.objects.get(objId);
            if (obj) {
                objData.x = obj.x;
        ...
        // Aggiorna anche la UI delle proprietà se il pannello è visibile
    ...
    applyFrame(frameIndex) {
        ...
        // Applica le proprietà del frame agli oggetti
        frame.objects.forEach((objData, objId) => {
            const obj = tab.objects.get(objId);
            if (obj) {
    ...
    playAnimation() {
        ...
        const animate = () => {
            ...
            for (let i = 0; i < tab.frames.length; i++) {
                ...
                // Se il frame ha un timestamp <= tempo trascorso e non è ancora stato applicato in questo ciclo
                if (elapsed >= frame.timestamp) {
                    // Verifica se questo frame è "nuovo" rispetto all'ultimo frame applicato
    ...
    snapObjectsToGrid(gridSize = 20) {
        ...
        objects.forEach(object => {
            ...
            // 2. Calcola la nuova posizione arrotondata al multiplo più vicino di gridSize
            ...
            // ⚠️ Nota: Se la tua classe SchemaEditor gestisce un modello di dati interno
            // (ad esempio, un array di oggetti con proprietà `x` e `y`),
    ...
    showContextMenu(e, objectId, type) {
        ...
        if (type === 'object') {
            ...
            menuItems = [
                { label: '✂️ Taglia', action: () => this.cutSelected(), shortcut: 'Ctrl+X' },
                { label: '📋 Copia', action: () => this.copySelected(), shortcut: 'Ctrl+C' },
                { separator: true },
                { label: '⬆️ In primo piano', action: () => this.bringToFront(), shortcut: 'Ctrl+]' },
                { label: '⬇️ In background', action: () => this.sendToBack(), shortcut: 'Ctrl+[' },
                { separator: true },
                ...
                { separator: true },
                { label: '🗑️ Elimina', action: () => this.deleteSelected(), shortcut: 'Canc' }
            ];
        } else if (type === 'arrow') {
            const arrow = this.getCurrentTab().arrows.get(objectId);
            menuItems = [
                { label: '━ Lineare', action: () => this.changeArrowType('linear') },
                { label: '⌢ Curva', action: () => this.changeArrowType('curved') },
                { label: '⚡ Zigzag', action: () => this.changeArrowType('zigzag') },
                { separator: true },
                { label: arrow?.dashed ? '━ Linea continua' : '╌ Linea tratteggiata', action: () => this.toggleDashedArrow() },
                { separator: true },
                { label: '🗑️ Elimina', action: () => this.deleteSelected(), shortcut: 'Canc' }
            ];
    ...
    changeFreehandOpacity(opacity) {
        ...
        if (freehand) {
            ...
            this.saveState(`Modificata opacità disegno a mano libera ${freehand.id} a ${opacity}`);
        }
    }
    ...
    changeSelectedObjectsOpacity(opacity) {
        ...
        this.saveState(`Modificata opacità oggetti selezionati a ${opacity}`);
    }
    ...
    changeFreehandColor(color) {
        ...
        if (freehand) {
            ...
            // ✅ Ri-renderizza completamente il disegno
            this.renderFreehand(freehand);
    ...
    saveWorkout(fileName) {
        ...
        // ✅ Leggi i campi globali dal modal
        ...
        // ✅ Crea il JSON completo con i nuovi campi
        const workoutData = {
            version: '1.1',
            savedDate: new Date().toISOString(),
            author: this.currentUser,
            objective: globalObjective,        // ✅ NUOVO
            observations: globalObservations,  // ✅ NUOVO
            tabs: allTabs
        };
    ...
    loadWorkout(event) {
        ...
        reader.onload = (e) => {
            try {
                ...
                // ✅ Mostra obiettivo e osservazioni se presenti
                let confirmMessage = `Questo caricamento sostituirà tutti gli schemi attuali con ${workoutData.tabs.length} schemi dal file.`;

                if (workoutData.objective) {
                    confirmMessage += `\n\n📋 Obiettivo: ${workoutData.objective}`;
                }

                if (workoutData.observations) {
                    confirmMessage += `\n\n📝 Osservazioni: ${workoutData.observations}`;
                }
                ...
                // ✅ Salva i dati globali per uso futuro (opzionale)
                ...
                workoutData.tabs.forEach((tabData, index) => {
                    ...
                    // ✅ Gestisci freehands se presenti
                    if (tabData.freehands) {
                        tabData.freehands.forEach(([id, freehand]) => tab.freehands.set(id, freehand));
                    }
                ...
                console.log(`✅ Allenamento caricato: ${workoutData.tabs.length} schemi`);
                console.log(`📋 Obiettivo: ${workoutData.objective || 'Non specificato'}`);
                console.log(`📝 Osservazioni: ${workoutData.observations || 'Nessuna'}`);
    ...
    async fetchLibraryWorkouts() {
        ...
        try {
            ...
            if (!response.ok) {
                // Lancia un errore se la risposta HTTP non è 200 (es. 404, 500)
            ...
            const listElement = document.getElementById('workoutList');
            if (listElement) {
                listElement.innerHTML = '<li style="color: orange; padding: 10px; cursor: default;">⚠️ **FALLBACK:** Server non raggiunto. Caricati esempi locali.</li>';
            }
        }
    ...
    async loadWorkoutFromBackend(filename) {
        ...
        if (this.librarySource === 'fallback') {
            ...
            // Se la risposta è OK, restituisci i dati dal server
    ...
    initializeEventListeners() {
        ...
        document.getElementById('spriteAnimationFPS').addEventListener('input', (e) => {
            // Se l'animazione è già in corso, riavviala con il nuovo FPS
    ...
    showCanvasContextMenu(e) {
        ...
        const menuItems = [
            { label: '📋 Incolla', action: () => this.paste(), shortcut: 'Ctrl+V', disabled: !this.clipboard },
            { separator: true },
            { label: '🔄 Seleziona tutto', action: () => this.selectAll(), shortcut: 'Ctrl+A' },
            { separator: true },
            { label: '↶ Annulla', action: () => this.undo(), shortcut: 'Ctrl+Z' },
            { label: '↷ Ripeti', action: () => this.redo(), shortcut: 'Ctrl+Y' }
        ];
    ...
    selectArrow(arrowId) {
        this.deselectAll(); // ✅ Questo è già presente
        ...
        if (arrowElement) {
            ...
            // ✅ VERIFICA CHE CI SIA QUESTO CODICE
    ...
    changeSelectedObjectsOpacity(opacity) {
        ...
        this.saveState(`Modificato opacità di ${this.selectedObjects.size} oggetti`);
        this.updateCurrentFrame();
    }
    ...
    changeArrowOpacity(opacity) {
        ...
        if (arrowData) {
            ...
            this.saveState(`Modificato opacità freccia ${arrowData.from.objectId}-${arrowData.from.position} a ${arrowData.to.objectId}-${arrowData.to.position}`);
        }
    }
    ...
    copySelected() {
        ...
        try {
            ...
            // ✅ MODIFICA: Crea una mappa per tracciare gli ID originali
            ...
            tab.arrows.forEach(arrow => {
                if (this.selectedObjects.has(arrow.from.objectId) &&
                    ...
                    const newArrow = {
                        ...
                        from: { ...arrow.from },
                        to: { ...arrow.to },
                        // ✅ CRITICO: Copia profonda del punto di controllo
                        controlPoint: arrow.controlPoint ? {
                            x: arrow.controlPoint.x,
                            y: arrow.controlPoint.y
                        } : null,
                        // ✅ AGGIUNTO: Salva gli ID originali per ricalcolare il punto di controllo
            ...
            this.clipboard = {
                ...
                // ✅ AGGIUNTO: Salva la mappa degli ID originali
                originalIdMap: originalIdMap
            };

            console.log(`✅ Copiati: ${copiedObjects.length} oggetti, ${copiedArrows.length} frecce, ${copiedFreehands.length} disegni`);
    ...
    paste() {
        ...
        // ✅ MODIFICA COMPLETA: Incolla frecce con punto di controllo corretto
        this.clipboard.arrows.forEach(arrow => {
            ...
            if ((newFrom.objectId || (newFrom.x !== undefined)) &&
                ...
                if (arrowData && arrow.controlPoint) {
                    // ✅ CRITICO: Sposta il punto di controllo mantenendo la distanza relativa
        ...
        console.log(`✅ Incollati: ${this.clipboard.objects.length} oggetti, ${this.clipboard.arrows.length} frecce, ${this.clipboard.freehands?.length || 0} disegni`);
    }
    ...
    createTabElement(id, name) {
        ...
        tab.innerHTML = `<input class='no-style tab-title' value='${name}'/><span class="tab-close">×</span>`;
    ...
    updateTabName(tabId, newName) {
        ...
        if (tab) {
            tab.name = newName;
            // Se è il tab attivo, aggiorna anche il titolo dello schema se vuoto
    ...
    loadTabState() {
        ...
        // ✅ NUOVO: Ripristina dimensioni custom se presenti
        if (tab.canvasSize === 'custom' && tab.customWidth && tab.customHeight) {
            canvas.style.width = tab.customWidth + 'px';
            canvas.style.height = tab.customHeight + 'px';
        }

        // ✅ AGGIUNGI TIMEOUT per aggiornare handle dopo il caricamento
        setTimeout(() => {
            this.updateAllResizeHandles();
        }, 150);
    ...
    `;
        ...
        <div><strong>📦 Oggetti:</strong> ${state.objects?.length || 0}</div>
        <div><strong>➡️ Frecce:</strong> ${state.arrows?.length || 0}</div>
        <div><strong>✏️ Disegni:</strong> ${state.freehands?.length || 0}</div>
        <div><strong>📝 Descrizione:</strong> ${state.description || 'Nessuna descrizione'}</div>
        ${index === tab.historyIndex ? '<div style="color: #28a745; font-weight: bold;">✅ Stato Corrente</div>' : ''}
    `;
    }
    ...
    async loadSchema(source) {
        ...
        tab.timing = data.timing || 10;      // ✅ AGGIUNTO
        tab.rec = data.rec || 60;            // ✅ AGGIUNTO                
    ...
    async exportSchema() {
        ...
        try {
            ...
            const exportCanvas = await html2canvas(canvas, {
                ...
                onclone: (clonedDoc) => {
                    ...
                    if (clonedCanvas) {
                        ...
                        // ✅ AGGIUNTO: Forza SVG con posizioni assolute per freehands
            ...
            tab.arrows.forEach(arrow => this.renderArrow(arrow));
            // ✅ AGGIUNTO: Re-renderizza i freehands normalmente
            tab.freehands.forEach(freehand => this.renderFreehand(freehand));
    ...
    // Metodo exportToPDF aggiornato per utilizzare il canvas già esportato correttamente
    ...
    handleKeyboard(e) {
        ...
        if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
            ...
            const moveStep = e.shiftKey ? 10 : 1; // Shift per movimento più veloce
    ...
    selectAll() {
        ...
        // ✅ AGGIUNTO: Seleziona anche tutti i disegni a mano libera
        ...
        // Mostra il centro di rotazione del gruppo se ci sono più oggetti
        if (this.selectedObjects.size > 1) {
            this.showGroupRotationCenter();
        }
    ...
    prepareCanvasForExport() {
        ...
        // ✅ AGGIUNTO: Re-renderizza tutti i disegni a mano libera
    ...
    async exportWorkoutToPDF() {
        ...
        for (const [tabId, tab] of this.tabs) {
            ...
            // Serie, Ripetizioni, Recupero e Timing (più compatti)
            doc.setFontSize(fontNormalSize);
            doc.setFont(undefined, 'normal');
    ...
    async exportFormationSheet() {
        ...
        for (let set = 1; set <= 5; set++) {
            ...
            // Rete (linea centrale più spessa)
            doc.setLineWidth(1);
