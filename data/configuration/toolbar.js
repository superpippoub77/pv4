...
const toolbarTopConfig = [
...

    {
        ...
        items: [
            ...
            { type: "input", inputType: "checkbox", id: "enableObjectDepth", text: "3D Depth", i18n: "btn_3d_depth", titleI18n: "btn_3d_depth", onChange: (editor, event) => { editor.objectDepthEnabled = event.target.checked; document.getElementById('enableObjectDepth')?.classList.toggle('active', event.target.checked); try { editor.saveUserPref('objectDepthEnabled', event.target.checked); } catch (err) { } try { editor.updateAllObjectTransforms(); } catch (err) { /* ignore */ } } },
            { type: "input", inputType: "checkbox", id: "autoObjectDepth", text: "Profondità auto", i18n: "btn_auto_depth", titleI18n: "btn_auto_depth", onChange: (editor, event) => { editor.objectDepthAuto = event.target.checked; document.getElementById('autoObjectDepth')?.classList.toggle('active', event.target.checked); try { editor.saveUserPref('objectDepthAuto', event.target.checked); } catch (err) { } try { editor.updateAllObjectTransforms(); } catch (err) { /* ignore */ } } },
            { type: "select", id: "objectDepthPreset", options: [["0", "none"], ["8", "small"], ["20", "medium"], ["40", "large"], ["custom", "custom"]], onChange: (editor, value) => { if (value !== 'custom') { editor.objectDepthPx = parseInt(value); const el = document.getElementById('objectDepthCustom'); if (el) el.value = editor.objectDepthPx; try { editor.saveUserPref('objectDepthPx', editor.objectDepthPx); } catch (err) { } try { editor.updateAllObjectTransforms(); } catch (err) { } } } },
            { type: "input", inputType: "number", id: "objectDepthCustom", text: "Depth(px)", i18n: "lbl_depth_px", min: "0", max: "200", value: "20", onInput: (editor, event) => { editor.objectDepthPx = parseInt(event.target.value) || 0; try { editor.saveUserPref('objectDepthPx', editor.objectDepthPx); } catch (err) { } try { editor.updateAllObjectTransforms(); } catch (err) { } } },
